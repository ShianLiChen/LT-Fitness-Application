...
def proxy_search_exercises():
    ...
    # --- Fallback Data (Use this if API fails) ---
    ...
    # 1. Try the External API First
    if params: # Only call API if we actually have search terms
        try:
            ...
            if response.status_code == 200:
                ...
                # Print error but DON'T fail the request, proceed to fallback
    ...
    # 2. Fallback Logic (Local Search)
    # Filter the local list based on the search params
    filtered_results = []
    for ex in local_fallback_exercises:
        # Check Muscle (if specified)
...
def proxy_generate_workout():
    ...
    try:
        ...
        # This ensures your demo works even if Ollama is down/slow
        import random
        mock_exercises = [
